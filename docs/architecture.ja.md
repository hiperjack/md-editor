...
- **プレゼン表示（スライドショー）**: `Ctrl+Shift+P`（またはタブ右クリック →「プレゼン表示を開く」、ツールバーボタン）で、HTML 出力と*同一*のレンダリングパイプラインから組み立てた読み取り専用のプレゼンタブを開く（スライドは文書プレビューと同じ見た目）。文書は `<hr>`（`---`）を境界に 16:9 スライドへ分割し、区切り線が無い場合は H1/H2 見出しで分割する。各スライドは 3 ゾーン固定レイアウト（タイトル＝先頭見出し / メッセージ＝直後の最初の段落 / 本文＝それ以降。本文は zoom で fit し、下限を下回るとゾーン内スクロール）。デッキ（サムネ一覧＋主ビュー）/ 一覧（グリッド）/ 全画面発表 の 3 ビューが同一の 16:9 キャンバスを共有する。レーザーポインタ（`L`）と「このページから発表」にも対応。表示時は HTML プレビューのズーム率を引き継ぐ
