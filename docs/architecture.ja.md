...
- **プレゼン表示（スライドショー）**: `Ctrl+Shift+P`（またはタブ右クリック →「プレゼン表示を開く」、ツールバーボタン）で、HTML 出力と*同一*のレンダリングパイプラインから組み立てた読み取り専用のプレゼンタブを開く（スライドは文書プレビューと同じ見た目）。文書は `<hr>`（`---`）を境界に 16:9 スライドへ分割し、区切り線が無い場合は H1/H2 見出しで分割する。各スライドは 3 ゾーン固定レイアウト（タイトル＝先頭見出し / メッセージ＝直後の最初の段落 / 本文＝それ以降。本文は zoom で fit し、下限を下回るとゾーン内スクロール）。デッキ（サムネ一覧＋主ビュー）/ 一覧（グリッド）/ 全画面発表 の 3 ビューが同一の 16:9 キャンバスを共有する。レーザーポインタ（`L`）と「このページから発表」にも対応。表示時は HTML プレビューのズーム率を引き継ぐ。付属の Claude Code スキル（`skills/presentation-md`）が、スライドの作法（`---` で区切る／見出し→キーメッセージ段落→本文）をまとめており、このビュー向けに最適化した Markdown の作成・整形を支援する
- **Claude チャットパネル（オプション、v2.0〜）**: ローカルの Claude Code CLI（`claude -p --output-format stream-json --include-partial-messages`、サブスクリプション認証 — 子プロセスの環境から `ANTHROPIC_API_KEY` を明示的に除外）を使う右サイドのチャットパネル。マスタースイッチは設定「Claude チャットを使う」（既定オフ。オフ時はツールバーボタン・メニュー項目ごと非表示）。詳細:
  - 毎メッセージ、アクティブタブの**全文バッファ**（未保存編集込み）を `<document>` タグで、エディタの選択範囲を `<selection>` タグで同梱し、stdin 経由で渡す（Windows のコマンドライン長制限を回避）
  - 編集提案は行頭の `<mdedit-proposal>` マーカーで受信（貪欲抽出、`src/chat-proposal.ts`）し、依存なしの Myers 行 diff（`src/diff.ts`）でプレビュー表示。適用は `EditorHost.setMarkdown` で**エディタバッファへ**行い、ディスクは触らない。「適用 / 新規タブへ / 破棄」の3択で、提案表示後にユーザーが本文を編集していた場合は diff を取り直して再確認させる
  - ファイル/シェル系ツールは常時無効（`--tools ""`）。「Web 検索を許可」オン時のみ WebSearch/WebFetch を有効化＋自動許可。`--strict-mcp-config --setting-sources ""` でユーザー環境の hooks/プラグインから隔離
  - 会話（最大20件、`--resume` 用のセッションID込み）はウィンドウラベル単位（main のみ）で `localStorage` にアーカイブし、「履歴」から再開できる。パネルは起動時常に非表示・新しい会話で開始
  - 「プレゼン変換」ボタン（または `/presentation` コマンド）は、同梱の `skills/presentation-md/SKILL.md`（Vite の `?raw` でビルド時に焼き込み）をそのメッセージのプロンプトに同梱する
  - Rust 側（`src-tauri/src/chat.rs`）: 1メッセージ=1プロセス。中継スレッド3本（stdin ライター / stdout の NDJSON を `chat-stream` イベントで1行ずつ中継 / stderr は末尾8KBのみ保持）、完了は `chat-done`、キャンセルは Windows では `taskkill /T`。**システムプロンプトは1行厳守** — Rust は `.cmd` シム向けに改行入り引数を拒否する（BatBadBut 対策）ため、npm 版 CLI で起動不能になる
...
- **GFM対応**: テーブル、タスクリスト、打ち消し線、GitHub アラート、目次（`[[toc]]`）、見出しアンカー（CommonMark + GFM プリセット）
  - 表の挿入はツールバーの表ボタン（ホバー/クリック）または書式メニューから、PowerPoint 風の 10列×8行グリッドピッカーで行列数を選ぶ（行数はヘッダ行を含む）
  - 表の最終セルの文末で `Tab` を押すと末尾に行を追加し、新行の先頭セルへ移動する（文中の `Tab` はセル内入力のまま）
  - 列・行選択時の操作ポップアップは、表が文書の上端付近にあるとき自動でハンドルの下側に反転表示する（エディタ領域外への見切れ防止）
  - エディタ上の表は `table-layout: auto` で描画し、列幅はセル内の文字量に応じて配分する（prosemirror-tables 既定の `fixed` では全列が等分になる）。HTML 出力・プレビュー側と同じ見え方になる。GFM には列幅を表す構文がないため手動での列幅指定は不可
  - Excel / PowerPoint / Word からコピーした表（クリップボードの `text/html`）は、画像ではなく GFM の表として貼り付ける。先頭行はヘッダ行へ昇格し、PowerPoint / Word のようにセル内の各行が別々の `<p>` になっている場合は `<br>` 区切りの 1 段落へまとめてから解析する（`table-paste.ts`。まとめないと 2 行目以降が隣の列へ押し出される）
...
`Ctrl+Shift+P` で開く。プレゼンタブにフォーカスがある間、以下のキーが有効。

| ショートカット | 動作 |
|---|---|
| `Ctrl+Shift+P` | 現在の文書のプレゼン表示を開く |
...
| `Tab` / `Shift+Tab`（表内） | 次 / 前のセルへ移動 |
| `Tab`（表の最終セルの文末） | 末尾に行を追加して新行の先頭セルへ |
...
│   ├── table-paste.ts           # 外部アプリの表貼り付け前処理（セル内複数段落を <br> 区切りの 1 段落へ）
...
│   ├── chat-panel.ts            # Claude チャットパネル（右サイドバー）: ストリーミング・提案カード・履歴
│   ├── chat-proposal.ts         # 提案マーカーの抽出・サニタイズ（純関数）
│   ├── diff.ts                  # 依存なしの行 diff（Myers）。提案プレビュー用
...
│   ├── presentation.ts          # プレゼン表示（スライド分割・デッキ/一覧/全画面・レーザー）
...
│   │   ├── chat.rs              # Claude Code CLI 連携（起動・NDJSON中継・キャンセル）
│   │   ├── recent.rs            # 最近使ったファイル
...
`presentation.ts` は exporter が生成する文書 HTML（`<main class="document">`）を再利用するため、スライドは HTML 出力とピクセル単位で一致する。分割は生 Markdown ではなく*レンダリング済み DOM*に対して行う（生テキスト分割は `text\n---` が setext H2 になる等で壊れるため）。境界はまず `<hr>`、区切り線が無ければ H1/H2 にフォールバックする。各スライドの子要素は 3 ゾーン（タイトル / メッセージ / 本文）へ割り当て、本文は CSS `zoom` で fit し、下限を下回るとゾーン内スクロールへ切り替える。デッキ・一覧・全画面の各ビューは同一の 16:9 キャンバスをマウントし、外側のレイアウトだけを差し替える。プレゼン操作バーは登録済みスロット経由でアプリのツールバーへ差し込む（`presentation.ts ↔ main.ts` の循環 import を避けるためコールバック方式）。現在のスライド番号は再生成をまたいでタブごとに記憶する。
